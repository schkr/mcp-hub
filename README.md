...
This dual-interface approach means you can manage servers through the Hub's UI while MCP clients (Claude Desktop, Cline, etc.) only need to connect to one endpoint (`127.0.0.1:37373/mcp`) to access all capabilities. The HTTP server listens on 127.0.0.1 (localhost) only, so it is not reachable from other machines. Implements [MCP 2025-03-26](https://modelcontextprotocol.io/specification/2025-03-26) specification.
